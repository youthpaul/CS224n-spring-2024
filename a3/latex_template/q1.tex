...
\end{lstlisting}
    ...
    \ifans{
        \newline
        First, we define the attention computation as:
        $$
            \alpha_{t, i} = \frac{\exp(\be_{t, i})}{\sum_{1 \le j \le m} \exp(\be_{t, j})}
        $$
        , where $\be_{t, i}$ represents the attention score of the $t^{th}$ decoded hidden state attending to the $i^{th}$ encoded hidden state. If $\be_{t, i}$ is set to $-\infty$, the attention weight \textbf{becomes zero} after applying the softmax.\newline
        We mask encoded hidden states to prevent the query (i.e., the $t^{th}$ decoded hidden state) from attending to the \textbf{padded positions}.
    }
...
\end{lstlisting}
    ...
    \ifans{
    \newline
        The BLEU on the test set is $19.741884$.
    }
    ...
    \ifans{
        \newline
        Dot product attention involves straightforward vector dot product operations, making it computationally efficient and fast. However, because it lacks additional trainable parameters, it may struggle with capturing the most relevant attention positions in more complex contexts, limiting the model flexibility.
    }
    ...
    \ifans{
        \newline
        Additive attention introduces a non-linear transformation in the attention computation, which makes it more flexible for capturing complex relationships between query and key pairs. However, this added complexity increases computational overhead and, in some cases, can lead to overfitting, especially when dealing with limited data.
    }
